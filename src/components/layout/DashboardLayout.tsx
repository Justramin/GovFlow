...
export function DashboardLayout({ portalName, roleName, basePath, children }: DashboardLayoutProps) {
  ...
  useEffect(() => {
    // Read mock user details if available
    const getCookieValue = (name: string) => {
      const match = document.cookie.match(new RegExp('(^| )' + name + '=([^;]*)'));
      return match ? decodeURIComponent(match[2]) : null;
    };
    
    const userCookie = getCookieValue("govflow-mock-user");
    if (userCookie) {
      try {
        const user = JSON.parse(userCookie);
        setUserName(user.name || getDemoName(roleName));
      } catch (e) {
        setUserName(getDemoName(roleName));
      }
    } else {
      setUserName(getDemoName(roleName));
    }
  ...
  const getDemoName = (role: string) => {
    switch (role) {
      case "STATE_ADMIN": return "Rajesh Kumar (State Sec.)";
      case "DISTRICT_ADMIN": return "Biju Varghese (District Admin)";
      case "DIVISION_ADMIN": return "Sajeev M. (Division Sec.)";
      case "UNIT_ADMIN": return "Anilkumar P. (Unit Sec.)";
      default: return "Administrator";
    }
  };

  const sidebarItems = [
    { name: "Overview", icon: <LayoutDashboard size={20} />, href: basePath },
    { name: "Members", icon: <Users size={20} />, href: `${basePath}/members` },
    { name: "Notices", icon: <Bell size={20} />, href: `${basePath}/notices` },
    { name: "Circulars", icon: <FileText size={20} />, href: `${basePath}/circulars` },
    { name: "Meetings", icon: <Calendar size={20} />, href: `${basePath}/meetings` },
    { name: "Attendance", icon: <CheckSquare size={20} />, href: `${basePath}/attendance` },
    { name: "Reports", icon: <BarChart3 size={20} />, href: `${basePath}/reports` },
  ];

  const handleLogout = () => {
    // Clear cookies
    document.cookie = "govflow-mock-role=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC;";
    document.cookie = "govflow-mock-user=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC;";
    
    // Redirect to matching login page
        ...
        <div className="p-6 flex flex-col h-full">
          <div className="flex items-center gap-3 mb-10 px-2">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">{portalName}</span>
            </div>
          </div>
          ...
          <div className="pt-6 border-t border-gray-100 space-y-1">
            <Link href={`${basePath}/config`} className="flex items-center gap-3 px-4 py-3.5 rounded-xl text-gray-600 hover:bg-gray-50 transition-all font-medium">
              <Settings size={20} />
              Config
            </Link>
            <button 
              onClick={handleLogout}
              className="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-red-500 hover:bg-red-50/50 transition-all font-medium text-left cursor-pointer"
            >
              <LogOut size={20} />
              Logout
            </button>
        ...
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-gray-100 sticky top-0 z-30 px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden sm:flex flex-col items-end mr-2">
              ...
              <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">{roleName.replace("_", " ")}</span>
            </div>
